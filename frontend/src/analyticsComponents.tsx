...
export function InterpretationBanner({
  ...
  return (
    <section aria-live="polite" style={{ ...cardStyle(tone), padding: "1rem 1.1rem", display: "grid", gap: "0.45rem" }}>
      ...
      <h2 style={{ margin: 0, fontSize: "clamp(1.1rem, 2vw, 1.45rem)", color: "#f7f2e8" }}>{title ?? defaultTitle}</h2>
      <p style={{ margin: 0, color: "#ddd3c3", lineHeight: 1.65 }}>{description ?? defaultDescription}</p>
    </section>
  );
}
    ...
    <section style={{ ...cardStyle("neutral"), padding: "1rem", display: "grid", gap: "0.8rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
        <div style={{ display: "grid", gap: "0.2rem" }}>
          ...
          <h3 style={{ margin: 0, fontSize: "1.05rem", color: "#f7f2e8" }}>{title}</h3>
        </div>
        <button
          type="button"
      ...
      {open ? (
        <div id={panelId} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "0.75rem" }}>
          {parameters.map((parameter) => (
            <article key={parameter.key} style={{ borderRadius: "18px", border: "1px solid rgba(240, 187, 116, 0.18)", background: "rgba(9, 13, 18, 0.42)", padding: "0.9rem", display: "grid", gap: "0.35rem" }}>
              <div style={{ fontWeight: 800, color: "#f7f2e8" }}>{parameter.label}</div>
              <div style={{ color: "#ddd3c3", lineHeight: 1.6 }}>{parameter.explanation}</div>
    ...
    <section style={{ ...cardStyle("neutral"), padding: "1rem", display: "grid", gap: "0.8rem" }}>
      ...
      <div style={{ display: "grid", gap: "0.65rem", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
        {categories.map((category) => (
          <article key={category.name} style={{ borderRadius: "18px", padding: "0.9rem", border: "1px solid rgba(240, 187, 116, 0.18)", background: "rgba(11, 15, 22, 0.42)", display: "grid", gap: "0.3rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem", alignItems: "center" }}>
              <strong style={{ color: "#f7f2e8" }}>{category.name}</strong>
              {category.coefficient !== undefined ? (
                <CoefficientBadge value={category.coefficient} metric="assortativity" size="sm" />
              ) : null}
            </div>
            <div style={{ color: "#ddd3c3", lineHeight: 1.55 }}>
            ...
            <div style={{ color: "#c8bea8", fontSize: "0.92rem" }}>
