...
Az inicializálás során minden törzshöz egy \textit{Genome::new(11,~7)} struktúra jön létre -
...
\subsection*{Gradiensmentes evolúció backpropagation helyett}
...
a \textit{genome.compile()} hívás egy \textit{CompiledGenome}-ot állít elő,
amely a topológiai előre-átmeneti lépéstervet tárolja.
A \textit{CompiledGenome::activate(\&inputs[0..11])} meghívása minden tickben elvégzi
...
és polity-fúziókor (\textit{Genome::inherit\_from()} hívásakor) érvénytelenítődik,
...
A \textit{CompiledGenome::activate()} hívás 11~értékből álló vektort kap.
...
A \textit{nearest\_enemy} és \textit{nearest\_ally} jelek \([0,\,1]\) tartományba
...
A hálózat kimenete \textit{tanh} aktivációs függvényen keresztül keletkezik,
...
\begin{itemize}
  \item \textbf{Opportunista háború:} \textit{raid\_drive}~$> 0{,}35$ \textit{és}
    \textit{food\_ratio}~$> 0{,}5$ \textit{és} gyengébb szomszéd létezik
    $\Rightarrow$ \textit{AtWar} állapotátmenet, céltörzs = leggyengébb
    szomszéd (\textit{pop~$\times$~a\_combat} alapján).
  \item \textbf{Migráció:} \textit{migration\_drive}~$> 0{,}55$ \textit{és}
    legalább 15~tick eltelt \textit{Settling} állapotban
    $\Rightarrow$ célpont kiválasztása (\textit{pick\_migration\_dest}),
    majd lépésenként hexagonális elmozdulás.
  \item \textbf{Szövetség-blokkolás:} \textit{isolation}~$> 0{,}75$
    $\Rightarrow$ a szövetségajánlat elutasítódik, még akkor is, ha
    \textit{goal\_drive} egyébként szövetséget indítana.
\end{itemize}

Az \textit{expansion\_speed} kimenet közvetlen numerikus módosító: a tile-igénylés
...
\begin{table}[h]
  ...
  \begin{tabular}{lrr}
    ...
    \hline
    \textit{migration\_drive} & 6\,526 & 38{,}4\% \\
    \textit{goal\_drive}      & 3\,127 & 18{,}4\% \\
    \textit{expansion\_speed} & 3\,060 & 18{,}0\% \\
    \textit{isolation}        & 1\,823 & 10{,}7\% \\
    \textit{resource\_drive}  & 1\,546 &  9{,}1\% \\
    \textit{raid\_drive}      &   634 &  3{,}7\% \\
    \textit{aggression}       &   258 &  1{,}5\% \\
    \hline
...
Az \textit{aggression} hajtóerő alacsony részesedése (1{,}5\%) nem hibajelzés:
a direkt agressziót a \textit{raid\_drive} és a \textit{goal\_drive} egyaránt
átfedik, más aktiválási feltétellel. A migráció dominanciája (38{,}4\%) pedig
összhangban van a flexset klaszterprofilok magas \textit{A\_map\_objective}
...
A \textit{server.js} már \([0,\,1]\) tartományra hozta az artifact értékeket,
majd a Rust engine ismét elosztotta őket 10-zel.
Ennek eredménye: az \textit{a\_combat} értéke a várható $\approx0{,}3$ helyett
...
A \textit{Settling} állapotból azonnali migrációs átmenet tüzelt, ha
\textit{migration\_drive}~$> 0{,}55$, és a törzs visszaérkezése után
ismét tüzelt. A törzsek körülbelül 15\%-a így soha nem maradt elég ideig
\textit{Settling} állapotban ahhoz, hogy területet igényeljen.

Harmadik hiba: túl konzervatív terjeszkedési küszöb.
A \textit{resource\_drive}~$< 0{,}25$ feltétel a törzsek $\approx63$\%-át zárta
...
\begin{itemize}
  ...
  \item \textbf{polity\_tier} - az elért politikai szint numerikusan kódolva:
    ...
    \textit{goal\_drive}), ezért önmagában is a koordinált stratégia jelzője.
  \item \textbf{war\_record} - a megnyert háborúk száma. Minden győzelem
    ...
    \textit{A\_combat} artifact értékéhez (ld. az artifact felhalmozódásáról szóló
    részt lejjebb).
\end{itemize}
...
Két törzs fuzionálhat, ha mindkettőre teljesül: \textit{goal\_drive}~$> 0{,}55$,
egyik sem tartózkodik \textit{AtWar} állapotban, és mindkettőre
\textit{isolation}~$< 0{,}75$.
Ez utóbbi feltétel közvetlen összefüggésben áll a 23.2~szekcióban ismertetett
\textit{isolation} hajtóerővel: az erősen izolacionista genomú törzs blokkolja
a szövetségajánlatot, ezért fuzionálásra sem kerülhet sor.

A fuzionálásból keletkező örökös genomja a \textit{Genome::inherit\_from(parent\_a,
...
Minden törzs nyilvántartja az \textit{entity\_id $\to$ [parent\_ids]} leképezést.
A seed-entitásoknál - vagyis az inicializáláskor közvetlenül klaszterprofilból
létrehozott törzsek esetén - a szülő \textit{u32::MAX} sentinel értéket kap;
ezek a leszármazási DAG gyökerei.

A TaskR1 implementációs futtatás (2026.~május~6.) vezette be a \textit{LineageRegistry}
struktúrát:
...
A REST endpointok:
\begin{itemize}
  \item \textit{GET /api/lineage/resolve/\{entity\_id\}} - a szülőlánc DAG visszaadása
  \item \textit{GET /api/lineage/seed/\{entity\_id\}} - visszakövetés az eredeti
    klaszterazonosítóig
  \item \textit{GET /api/lineage/stats} - az összes entitás száma és
...
Az \textit{A\_combat} érték nem marad rögzített az inicializálás utáni szinten.
...
egy alacsony \textit{A\_combat}-tal induló törzs - ha elegendő védelmi győzelmet arat -
...
A futtatás végén (tick=2538) mért \textit{A\_combat}~$= 0{,}98$ szinte pontosan
...
Az \textit{A\_risk} artifact (\ref{tab:neurosim-inputs}.~táblázat, 10.~index)
...
Az opportunista háború mechanizmusa célirányos: \textit{raid\_drive}~$> 0{,}35$
és \textit{food\_ratio}~$> 0{,}5$ és létezik a térképen gyengébb szomszéd
(alacsonyabb \textit{pop~$\times$~a\_combat} szorzattal) - ha mindhárom feltétel
teljesül, a törzs háborút indít a leggyengébb azonosított szomszéd ellen.
A szelektivitás szándékos: a magas \textit{raid\_drive} nem jelent vaktában
agresszív viselkedést, hanem kalkulált opportunizmust. Ez egyezik a 23.2~szekcióban
ismertetett \textit{raid\_drive}-\textit{aggression} különbségtétellel: az előbbi
...
viselkedési mintákhoz: a magas \textit{raid\_drive} és magas \textit{aggression}
kombinációja Hawk-típusú viselkedési pozícióként azonosítható - a törzs aktívan keresi
a konfliktust és maximalizálja az erőforrásfoglalást. A magas \textit{isolation} és magas
\textit{goal\_drive} kombinációja ezzel szemben Dove-típusú pozíciónak tekinthető:
...
ezt az eseményt a \textit{A\_combat} értékek is tükrözik: tribe\_596 védelmi
...
Tribe\_89 alacsony agresszióval (0{,}11) és magas \textit{raid\_drive}-val (0{,}84)
...
hogy a 23.2~szekcióban ismertetett \textit{isolation} hajtóerő (0{,}68)
