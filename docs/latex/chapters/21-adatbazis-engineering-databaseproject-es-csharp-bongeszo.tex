...
A séma normalizált: a meccsadatok nem ismétlődnek játékosonként, a résztvevői rekord tartalmazza a meccsre vonatkozó joinolt mezőket, a klasztertagság idegen kulcson keresztül kapcsolódik a játékoshoz és a klaszterhez. A \textit{CHECK} constraintek biztosítják, hogy az \textit{opscore} és \textit{feedscore} értékek a $[0,10]$ tartományban maradnak.

\subsection{Nézetek és összetett lekérdezések}

A \textit{docs/databaseproject/views\_queries.sql} fájl elemző nézeteket és lekérdezéseket tartalmaz. Ezek közé tartoznak:
\begin{itemize}
    ...
    \item \textbf{JSON-alapú metaadat-lekérdezések}: a klaszter \textit{summary\_json} és \textit{role\_json} mezőinek lekérdezése PostgreSQL JSON operátorokkal.
\end{itemize}

\subsection{Tárolt eljárások és triggerek}

A \textit{docs/databaseproject/procedures\_triggers.sql} tárolt eljárásokat és triggereket definiál. Ezek közé tartoznak:
\begin{itemize}
...
A \textit{docs/databaseproject/simple-interface/} könyvtárban egy ASP.NET Core webalkalmazás él, amelyet a Premade Graph SQLite adatbázisainak böngészésére terveztek. A böngésző \textit{Program.cs} fájlja mutatja, hogyan épül fel ez az eszköz.

\subsection{Adatbázis-generálás és importálás}

Az alkalmazás indulásakor automatikusan futtatja a \textit{ProjectImporter.Import()} folyamatot, amely a Premade Graph repositoryban lévő SQLite adatbázisból importál a böngésző saját, projektre generált SQLite állományába:

\begin{itemize}
    \item \textit{players}: opscore, feedscore, szerepkör, meccsszám;
    \item \textit{clusters}: klasztertípus, méret, metaadatok;
    \item \textit{cluster\_members}: tagsági rekordok;
    \item \textit{replay}: útkeresési mentések;
    \item \textit{replay\_run}: útkeresési futások.
\end{itemize}
...
Az alkalmazás tudatosan illeszkedik a Premade Graph több-adatkészletes architektúrájához. A \textit{DatasetCatalog.Load(root)} a repository \textit{datasets.json} regiszterét olvassa be, és a böngésző minden oldalán URL-paraméterként átadható a kívánt dataset azonosítója (\textit{?dataset=flexset}, \textit{?dataset=soloq}).
...
\begin{itemize}
    ...
    \item \textbf{Teljes értéknézet}: a \textit{/database/value} endpoint egy adott cella teljes értékét mutatja meg, különösen hasznos a \textit{summary\_json} és \textit{role\_json} típusú, hosszú JSON-oszlopoknál.
\end{itemize}

\subsection{Technikai architektúra}

Az alkalmazás egész programja egyetlen \textit{Program.cs} fájlban helyezkedik el, minimális top-level statement stílussal. Az ASP.NET Core pipeline konfigurálása, az útvonaldefiníciók és az adatbázis-lekérdezések egységes szerkezetben élnek. A renderelés szerver oldali HTML-generálással történik, nincs JavaScript frontend: a böngésző célja az áttekinthetőség és a bemutathatóság, nem egy SPA-szerű élmény.

A \textit{Microsoft.Data.Sqlite} NuGet csomag az adatbázis-elérés alapja, amely a SQLite adatbázisokhoz natív, közvetlen hozzáférést biztosít .NET-ből. A kapcsolatfelépítés és a lekérdezések a \textit{ProjectDatabase} osztályon keresztül vannak absztraháLva, ami megkönnyíti a dataset-váltás és az import-logika szétválasztását.

\section{A kiegészítő alprojekt helye a dolgozatban}

\subsection{Az alprojekt megvalósíthatósága a repositoryban}

A databaseproject nem puszta papír-terv: a \textit{schema.sql} futtatható, a \textit{views\_queries.sql} lekérdezései tesztelhetők PostgreSQL-ben, a C\# böngésző lefordul és futtatható a repository-ból. Mindez valódi, védhető mellékprojektté teszi.

Az akadémiai értéke kettős. Adatbázis-tervezési szempontból bemutatja, hogyan formalizálható ugyanaz a domainmodell egy erősebb, tárolt logikát támogató adatbázisrendszerben; ez az SQLite-on dolgozó fő rendszer korlátait is megvilágítja. Szoftverarchitektúra szempontból demonstrálja, hogy a Premade Graph domain-modellje természetesen leírható relációs sémával, tárolt eljárásokkal és triggerekkel, miközben az alapvető fogalmak (players, clusters, cluster\_members, opscore, feedscore) mindkét adatbázis-környezetben koherensek.
