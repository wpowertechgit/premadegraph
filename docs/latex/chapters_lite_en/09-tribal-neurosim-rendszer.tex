...
The ghost war specifically meant that wars belonging to finished and dead tribes never received a closed status: because no structured war-closing event existed, the system continued to track them as active, and their count accumulated in memory --- in the final state of the early prototype, a single living tribe faced more than a hundred active ghost wars. In the current architecture every decision is made in the Rust binary, and a tribe's elimination atomically closes all its active wars (\textit{WarCancelled}), so ghost wars cannot arise; the MonoGame client only renders the most recently received frame, and the simulation is reproducible from the same initial state at any point.
