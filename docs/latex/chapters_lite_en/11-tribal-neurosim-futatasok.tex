...
Tribes evolve through five polity levels: \textit{Tribe}, \textit{City}, \textit{Duchy}, \textit{Kingdom}, and \textit{Empire}. Each level-up requires meeting population, territory, and alliance thresholds; higher levels unlock greater maximum population and better combat efficiency, but also impose higher food-maintenance demands. Reaching Empire level is the victory condition: all four full runs and every run in the 103-seed sweep concluded with an Empire-level winner.

Combat outcomes are based on a Poisson process: the expected casualties are determined by the attacker's combat artifact and the contact zone (the number of adjacent tile pairs on the boundary between the two territories, $\lambda = a_{\text{combat}} \cdot n_{\text{contact}} \cdot 0{,}02$), but the concrete tick-level outcome is stochastic. The \textit{liveness-fix} (14 May 2026) corrected three numerical errors --- double normalisation of tribe statistics, oscillation of the migration drive, and an overly conservative expansion threshold --- which had previously made the simulation static. Separate from this is the determinism-bug fix: unordered SQLite loading of clusters produced different tick orderings and therefore different outcomes; after introducing \textit{clusters.sort\_by(id)}, re-runs produce bit-identical results. Detailed advancement conditions and iterative design decisions can be found in the extended version \cite{premadegraph-full}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{assets/neurosim/run67fl42-fl-s42-tick2487-last.png}
\caption{Flexset, seed=42, tick=2487: Tribe 596 (Warband/Combat) victory.}
\label{fig:run-flexset-42}
\end{figure}
...
The log files of the 103-seed sweep contain approximately 35\,000 lines of JSONL-format event logs per run. To aggregate the resulting data volume and identify the archetype distribution, an AI-based text analysis tool was used; the aggregated statistics were subsequently verified manually.
