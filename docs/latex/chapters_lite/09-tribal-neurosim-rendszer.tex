...
A kísérlet kerete exploratív transzfervizsgálat \cite{bonabeau2002}. Nem kísérel meg közvetlen előrejelzést a valódi meccskimenetelekre, és nem állít fel pszichológiai vagy szociológiai kauzalitást.
...
A \ref{fig:neurosim-architecture-graphviz} ábra a Tribal NeuroSim három rétegének hatáskör-megosztását mutatja be: Rust motor (autoritatív állapot), Node (közvetítő), MonoGame (megjelenítő).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{assets/diagrams/thesis_neurosim_architecture_graphviz.png}
...
A ghost war konkrétan azt jelentette, hogy a befejezett és a halott törzsekhez tartozó háborúk soha nem kaptak lezárt státuszt: mivel nem létezett strukturált háborúlezárási esemény, a rendszer ezeket továbbra is aktívként tartotta nyilván, és a számuk a memóriában halmozódott: a korai prototípus végállapotában egyetlen élő törzs állt szemben több mint száz aktív szellemháborúval. A jelenlegi architektúrában minden döntés a Rust binárisban születik, és a törzs felszámolása atomikusan zárja le az összes aktív háborút (\textit{WarCancelled}), így szellemháborúk nem keletkeznek; a MonoGame kliens csak a legutóbb kapott frame-et rendereli, és a szimuláció bármely pillanatban reprodukálható ugyanabból a kezdőállapotból.

\section{FrameV1 bináris protokoll}

A Rust szerver és a MonoGame kliens bináris WebSocket-kereteken kommunikálnak. Minden keret egy 40 bájtos \textit{TNS3} envelope-pal kezdődik (little-endian):

Az \ref{tab:framev1-envelope} táblázat a FrameV1 bináris WebSocket-keret 40 bájtos fejlécének mezőit mutatja be.
...
A bináris formátum döntése teljesítményi: a Rust szerver 40~ms alatt lekezel 599 törzs állapotát, miközben a MonoGame kliens aszinkron dekódol és renderel. A JSON-sorosítás elkerülése egy 599-törzsös világ esetén tickenként ~150–200~kB felesleges szöveges overheadt spórol meg.
